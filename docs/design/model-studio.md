...
The renderer-neutral `@voxel/model-studio-ui` package in `../voxel/tools/studio/shared-ui` is the required workbench rather than prior art to copy. It owns the exact top, shelf, stage, player, and inspector regions; the standard Examine, Build, Edit, Motion, and Notes capabilities; keyboard and ARIA behavior; scoped visual tokens; lifecycle disposal; and the shared shell baseline. This app supplies its gallery/genome stage and pane contents through that package's declared extension seams without copying or forking Townscaper's template, outer CSS, or interaction code. `townscaper/src/model-studio/` remains the proof that a game-specific renderer and harness can mount the shared shell: `window.harborformModelStudio` exposes catalog, camera, variation, recipe, and selected-part state, while `advanceTime(ms)` controls deterministic animation. Its editing is indirect — it emits an evolution brief for an agent to apply — whereas here the genome is the edit.

## Shared-shell profile and add-ons

The shared shell is reuse-first. A consumer using its V2 profile explicitly opts into the standard capabilities it actually supports; Examine is mandatory, and enabled standards always retain the canonical Examine → Build → Edit → Motion → Notes order even when intervening capabilities are omitted. A capability that the product does not implement is omitted, while a capability that is only temporarily unavailable remains present with an explanation and a recovery action.

A game-specific inspector capability is a declarative add-on with a stable, namespaced identifier such as `3d-maker:evolve`. Add-ons are appended after all enabled standard capabilities; they may not replace, reorder, or redefine a standard. The engine continues to own markup, layout, focus behavior, keyboard behavior, lifecycle, and visual tokens, while the consumer supplies only the add-on label, content, and corresponding harness command/state. A capability reused by more than one game graduates into the engine as a standard capability or shared add-on instead of being implemented twice.

The phase 1 3d-maker profile is Examine → Build → Edit → `3d-maker:evolve`. Motion is omitted until the product has genome-driven parametric motion to inspect, and Notes is omitted because phase 1 has no notes workflow. The `3d-maker:evolve` add-on holds the product-specific survivor/parent selection, mutation-temperature, and lineage controls; the gallery remains in the shared stage, the library remains in the shared shelf, and import/export remain shared top-level actions rather than becoming redundant add-ons.
...
6. UI over that same core, mounted into the shared five-region Model Studio shell with the declared phase 1 V2 profile rather than a local workbench copy.
