 ...
 * neither, and what this catches is the drift that matters most: a frame value
 * retuned, a panel tone moved, or the scene's environment level changed.
 ...
 *  Deep Blue's roughness is 0.42 and not the 0.3 round 7 fitted: the environment
 *  gained the veneer that darkens the profiles' cover glass, which took that
 *  rail down further than its panel, and `palette.ts`'s own row carries the
 *  ladder — 0.35 = 0.503, 0.42 = 0.469 against a 0.47 reference. */
const FRAME = {
  'cosmic-orange': { roughness: 0.62, clearcoat: 0.25, tint: 'cf6238' },
  'deep-blue': { roughness: 0.42, clearcoat: 0.18, tint: '323e57' },
 ...
 * the bounds are stated rather than inlined — matte is a roughness at or above
 * the floor, a dielectric a metalness below the ceiling, and no clearcoat is
 * what keeps a second, smoother mirror off a face the reference shows no
 * reflection on.
 *
 ...
 * only that the code agrees with itself. The rows solve to the reference's 0.83
 * of the frame's luma on the same row of the `top` view — measured 0.833 Cosmic
 * Orange, 0.831 Deep Blue, 0.836 silver — and a retune that moved any of these
 * hexes has to move this table with it.
 */
const MMWAVE = {
  roughness: 0.82,
  metalness: 0,
   ...
   * Darker than the frame wherever the frame's albedo has room under it — 0.578
   * on Cosmic Orange, 0.550 on silver — but *not* on Deep Blue, and that
   * exception is the measurement: that frame's albedo is the darkest of the
   * three (0.0481) and it is metal, so it renders at 118 luma where a diffuse
   * surface of the same albedo cannot. An insert landing the reference's step
   * there carries a hair more albedo than the frame does (0.0528 against
   * 0.0481) and still renders at 0.831 of it; making that row darker than its
   * frame costs the step — a two-point fit puts parity of albedo at about 0.79
   * rendered, 0.04 below the reference and the other two finishes.
   */
