...
// run too, and reads one named rect of each frame back through the same canvas.
 ...
 *  `left` carries no floor: see the header on why its flatness is physical. */
...
async function measureGlass(send, stop) {
  ...
  exceptions.length = 0;
  await open(send, `${BASE}/iphone.html?shot=1&view=${view}&color=${GLASS_COLOR}`, label);
...
async function main() {
  ...
  try {
    ...
    // The glass section: the cover glass on `front`, `right` and `left`, one
    // named rect each, on cosmic-orange. On for every run, like the port's.
    const glass = [];
